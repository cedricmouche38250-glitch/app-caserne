...
try:
    ...
    with st.container(border=True): # Affichage simple sans image pour tester st.subheader(f"Engin : {row['Engin']}") agent = row['Verificateur'] if pd.notnull(row['Verificateur']) else "À désigner" st.write(f"👤 Actuel : {agent}") 
